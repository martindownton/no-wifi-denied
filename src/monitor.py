...
class WiFiMonitor:
    ...
    def start_dnsmasq(self):
        ...
        try:
            ...
            subprocess.Popen(['dnsmasq', '-C', config_path, '-d'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    def enter_setup_mode(self):
        ...
        if self.nm.start_hotspot():
            ...
            if not self.portal_thread or not self.portal_thread.is_alive():
                self.portal_thread = threading.Thread(target=run_portal, kwargs={'port': 80}, daemon=True)
        ...
        while True:
            if not self.setup_mode:
                if self.nm.is_connected():
                    ...
                    logging.warning(f"Connectivity check failed ({self.failure_count}/{self.failure_threshold})")
                ...
                if creds:
                    logging.info(f"Received credentials for '{creds['ssid']}'. Attempting to connect...")
                    ...
                    if self.nm.connect_to_wifi(creds['ssid'], creds['password']):
                        ...
                        if creds.get('email'):
                            ...
                            self.notifier.send_connection_success(creds['email'], creds['ssid'], ip)
                    else:
                        ...
                        if creds.get('email') and self.nm.is_connected():
                            self.notifier.send_connection_failure(creds['email'], creds['ssid'])

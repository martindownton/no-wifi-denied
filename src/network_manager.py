...
class NetworkManager:
    ...
    def scan_wifi(self):
        ...
        try:
            ...
            logging.warning("nmcli not found or error. Returning mock data for UI testing.")
            return [
    ...
    def start_hotspot(self, password=None):
        ...
        try:
            ...
            cmd = ['nmcli', 'device', 'wifi', 'hotspot', 'ifname', self.interface, 
                   'con-name', self.hotspot_con_name, 'ssid', self.hotspot_ssid]
            if password:
            ...
            logging.warning(f"Failed to start hotspot (nmcli missing/error). Local testing mode: {e}")
            return True # Pretend it started for testing purposes if you are local

    def stop_hotspot(self):
        """Stops and removes the hotspot connection."""
        try:
            subprocess.run(['nmcli', 'connection', 'down', self.hotspot_con_name], 
                           stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            subprocess.run(['nmcli', 'connection', 'delete', self.hotspot_con_name], 
                           stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            return True
        except Exception:
            return False
    ...
    def connect_to_wifi(self, ssid, password):
        ...
        try:
            ...
            cmd = ['nmcli', 'device', 'wifi', 'connect', ssid, 'password', password, 'ifname', self.interface]
